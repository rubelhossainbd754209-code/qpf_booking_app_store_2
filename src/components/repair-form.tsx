...
export function RepairForm() {
  ...
  const brands = formOptions.brands;

  // Device types based on selected brand (from API)
  const getDeviceTypes = (brand: string): string[] => {
    return formOptions.deviceTypes[brand] || [];
  };

  // Device models based on selected device type (from API)
  const getDeviceModels = (brand: string, deviceType: string): string[] => {
    const key = `${brand}-${deviceType}`;
                  ...
                  render={({ field }) => (
                    <FormItem className="text-left">
                      ...
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        ...
                        <SelectContent className="max-h-60">
                          {getDeviceModels(formData.brand, formData.deviceType).map((model) => (
